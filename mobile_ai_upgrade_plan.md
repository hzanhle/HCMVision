...
- Chatbot hiện tại đang gọi tới Google Gemini qua API.
- **Cách nâng cấp:**
  - Sửa `ChatbotService.cs` trong Backend.
  - Thay thế thư viện/URL gọi Gemini bằng endpoint của Qwen-3 (tương thích OpenAI API chuẩn).
...
- **[Medium - Vừa]** Sửa `ChatbotService.cs`: Chuyển API gọi từ Google Gemini sang API OpenAI-compatible của Qwen-3 Cloud / Local. Tinh chỉnh prompt để Chatbot hiểu context dữ liệu trong DB.
