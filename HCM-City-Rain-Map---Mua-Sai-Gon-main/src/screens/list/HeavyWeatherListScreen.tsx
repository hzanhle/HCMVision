...
export default function HeavyWeatherListScreen({ navigation }: any) {
  const getCamerasByWeatherStatus = useAppStore(
    (s) => s.getCamerasByWeatherStatus,
  );
  const heavyWeatherCameras = getCamerasByWeatherStatus("heavy");
    ...
    <View className="flex-1 bg-gray-50">
      ...
      <SafeAreaView edges={["top"]}>
        <HStack className="items-center px-4 py-3 bg-white border-b border-gray-200">
          ...
          <Text className="text-sm text-gray-600">
            {heavyWeatherCameras.length} cameras
          </Text>
        </HStack>
      </SafeAreaView>

      <FlatList
        data={heavyWeatherCameras}
