...
export default function LightRainListScreen({ navigation }: any) {
  const getCamerasByWeatherStatus = useAppStore(
    (s) => s.getCamerasByWeatherStatus,
  );
  const lightRainCameras = getCamerasByWeatherStatus("light");
      ...
      <SafeAreaView edges={["top"]}>
        <HStack className="items-center justify-between px-4 py-3 bg-white border-b border-gray-200">
          ...
          <Text className="text-sm text-gray-600">
            {lightRainCameras.length} cameras
          </Text>
        </HStack>
      </SafeAreaView>

      <FlatList
        data={lightRainCameras}
