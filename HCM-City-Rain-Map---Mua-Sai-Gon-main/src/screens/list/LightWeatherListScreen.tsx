...
export default function LightWeatherListScreen({ navigation }: any) {
  const getCamerasByWeatherStatus = useAppStore(
    (s) => s.getCamerasByWeatherStatus,
  );
  const lightWeatherCameras = getCamerasByWeatherStatus("light");
  ...
  const EmptyComponent = () => (
    <Center className="flex-1 px-8 py-16">
      ...
      <Text className="text-base text-gray-600 text-center leading-6">
        No cameras detecting light weather
      </Text>
    </Center>
  );
  ...
  const HeaderComponent = () => (
    <HStack className="items-center mb-4">
      <Box className="w-3 h-3 rounded-full bg-blue-400 mr-2" />
      <Text className="text-sm text-gray-600">Light weather detected</Text>
    </HStack>
  );
      ...
      <SafeAreaView edges={["top"]}>
        <HStack className="items-center px-4 py-3 bg-white border-b border-gray-200">
          ...
          <Text className="text-sm text-gray-600">
            {lightWeatherCameras.length} cameras
          </Text>
        </HStack>
      </SafeAreaView>

      <FlatList
        data={lightWeatherCameras}
