...
export const areaService = {
  ...
  async getAreaById(id: string): Promise<ApiResponse> {
    try {
      const response = await apiCall(`/api/areas/${id}`);
      return { success: true, data: response };
    } catch (error: any) {
      return { success: false, error: error.message };
    }
  ...
  async getAreasByRainStatus(status: string): Promise<ApiResponse> {
    try {
      const response = await apiCall(`/api/areas?rainStatus=${status}`);
      return { success: true, data: response };
    } catch (error: any) {
      return { success: false, error: error.message };
    }
